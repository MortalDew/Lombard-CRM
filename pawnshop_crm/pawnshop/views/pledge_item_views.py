...
class PledgeItemCreateAjaxView(View):
    def post(self, request, *args, **kwargs):
        ...
        if invalid_fields:
            ...
            if (len(data.get('subcategory_pk')) < 3):
                pk_prec = get_object_or_404(Subcategory, pk=int(data.get('subcategory_pk')))
            else:
                i = True
                new_name = data.get('subcategory_pk')
                for result in Subcategory.objects.all():
                    if (result.name == new_name): i = False
                if (i):
                    pk_prec = Subcategory.objects.create(
                        name = data.get('subcategory_pk'),
                        category=get_object_or_404(Category, pk=int(data.get('category_pk'))),
                    )
                else: raise Http404('Matches existing.')
